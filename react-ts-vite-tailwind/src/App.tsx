...
import { Send, MessageCircle, Plus, Trash2, Moon, Sun, Mic } from "lucide-react";
...
if (!success) {
  ...
  const activeChatBgClass = isDarkMode ? 'bg-blue-900 border-blue-700' : 'bg-blue-50 border-blue-200';

  return (
    <div className={`flex h-screen ${bgClass} ${textClass} transition-colors duration-200`}>
      {/* Левая колонка - История чатов */}
      <aside className={`w-80 ${sidebarBgClass} border-r ${sidebarBorderClass} flex flex-col transition-colors duration-200`}>
        {/* Заголовок и кнопки */}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {chat.messages.length > 0 && (
                    <p className={`text-sm truncate mt-1 ${secondaryTextClass}`}>
                      {chat.messages[chat.messages.length - 1].sender === 'user' ? 'Вы: ' : 'Elli: '}
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div className={`border-b ${sidebarBorderClass} ${sidebarBgClass} p-4`}>
          ...
          <div className="flex items-center gap-2 mt-1">
            ...
            <span className={`text-sm ${secondaryTextClass}`}>
              {isLoading ? 'Elli печатает...' : 
               isConnected ? 'Готов к работе' : 'Не подключено'}
            </span>
          </div>
        </div>

        {/* Область сообщений */}
        <div className={`flex-1 overflow-y-auto p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'} transition-colors duration-200`}>
          {activeChat.messages.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              ...
              >
                <MessageCircle size={48} className={`mx-auto mb-4 ${isDarkMode ? 'text-gray-600' : 'text-gray-300'}`} />
                <p className="text-lg">Начните общение с Elli</p>
                <p className={`text-sm mt-2 ${secondaryTextClass}`}>Задайте вопрос или начните голосовой диалог</p>
              </motion.div>
            </div>
          ) : (
            <div className="space-y-4">
                ...
                >
                  ...
                  >
                    ...
                    <p className={`text-xs mt-1 ${
                      message.sender === 'user' ? 'text-blue-100' : secondaryTextClass
                    }`}>
              ...
              {isLoading && (
                ...
                >
                  <div className={`max-w-[70%] rounded-2xl px-4 py-3 ${messageBgClass} ${textClass} border ${messageBorderClass} rounded-bl-none`}>
        ...
        {/* Поле ввода */}
        <div className={`border-t ${sidebarBorderClass} ${sidebarBgClass} p-4 transition-colors duration-200`}>
          <div className="flex items-center gap-3">
            <input
              type="text"
              placeholder={isConnected ? "Введите сообщение..." : "Ожидание подключения к серверу..."}
